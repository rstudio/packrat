...
githubUpdates <- function(lib.loc = .libPaths()) {
  do.call(
    rbind,
    enumerate(lib.loc, function(lib) {
      ...
      if (!requireNamespace("httr")) {
        stop("Need package 'httr' to check for GitHub updates")
      }
      do.call(
        rbind,
        enumerate(DESCRIPTIONS, function(x) {
          url <- file.path(
          ...
          if (response$status == 403) {
            ...
            warning(
              "failed to get tracking information for GitHub package '",
              x[, "Package"],
...
bitbucketUpdates <- function(lib.loc = .libPaths()) {
  do.call(
    ...
    enumerate(lib.loc, function(lib) {
      ...
      if (!requireNamespace("httr")) {
        stop("Need package 'httr' to check for Bitbucket updates")
      }
      do.call(
        rbind,
        enumerate(DESCRIPTIONS, function(x) {
          ...
          if (response$status == 403) {
            ...
            warning(
              "failed to get tracking information for Bitbucket package '",
              x[, "Package"],
              "'; did its associated repository move?",
              call. = FALSE
            )
            ...
            ## Find the index of the response with the appropriate name
            index <- which(
              sapply(content$values, `[[`, "name") == x[, "RemoteRef"]
            )
            if (!length(index)) {
              warning(
                "no reference '",
  ...
  do.call(
    ...
    enumerate(lib.loc, function(lib) {
      ...
      DESCRIPTIONS <-
        Filter(
          function(x) {
            "RemoteType" %in% colnames(x) && x[, "RemoteType"] == "gitlab"
          },
          DESCRIPTIONS
        )
      if (!length(DESCRIPTIONS)) {
      ...
      if (!requireNamespace("httr")) {
        stop("Need package 'httr' to check for Gitlab updates")
      }
      do.call(
        rbind,
        enumerate(DESCRIPTIONS, function(x) {
          ...
          if (response$status == 403) {
            ...
            warning(
              "failed to get tracking information for Gitlab package '",
              x[, "Package"],
              "'; did its associated repository move?",
              call. = FALSE
            )
