getPackageDependencies <- function(
  ...
  deps <- unlist(lapply(pkgs, function(pkg) {
    ...
    pkgDescFile <- system.file('DESCRIPTION', package = pkg, lib.loc = lib.loc)

    # Get any packages available in local repositories
    localReposPkgPaths <- as.character(unlist(lapply(
      opts$local.repos(),
      function(x) {
    ...
    if (file.exists(pkgDescFile)) {
      ...
      if (length(allIdx) > 1) {
        warning(
          "Package '",
          pkg,
          "' found in multiple local repositories; ",
          "inferring dependencies from package at path:\n- ",
          shQuote(path)
        )
      }
      theseDeps <- combineDcfFields(
        as.data.frame(readDcf(file.path(path, "DESCRIPTION"))),
        fields
      )
    } else {
...
recursivePackageDependencies <- function(
  ...
  while (length(depsToCheck)) {
    newDeps <- getPackageDependencies(
      depsToCheck,
      lib.loc,
      available.packages,
      fields
