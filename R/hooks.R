...
makeCopyAvailablePackagesCacheCmd <- function(contrib.url, dir) {
  ...
  fromCmd <- paste0(
    "file.path(",
    paste(sep = ", ", surround(dir, with = "'"), surround(name, with = "'")),
    ")"
  )
...
buildSnapshotHookCall <- function(project, debug = FALSE) {
  ...
  repos <- gsub(
    "\"",
    "'",
    paste(deparse(getOption('repos'), width.cutoff = 500), collapse = ' ')
  )
  ...
  if (debug) {
    ...
    snapshotCmd <- paste0(
      "try(suppressMessages(packrat:::snapshotImpl(",
      snapshotArgs,
      ")), silent = TRUE)"
    )
  }
...
snapshotHookImpl <- function(debug = FALSE) {
  ...
  fullCmd <- paste(
    buildSnapshotHookCall(project, debug = debug),
    collapse = "; "
  )
