...
dir_copy <- function(
  ...
  if (!success) {
    stop("Couldn't create directory '", to, "'.")
  }

  # Get relative file paths
  files.relative <- list.files(
    from,
    all.files = all.files,
    full.names = FALSE,
    recursive = TRUE,
  ...
  if (!all(res)) {
    ...
    unlink(to, recursive = TRUE)
    stop(
      "Could not copy all files from directory '",
      from,
      "' to directory '",
      to,
      "'."
...
wrap <- function(x, width = 78, ...) {
  paste(
    strwrap(x = paste(x, collapse = " "), width = width, ...),
    collapse = "\n"
  )
}
...
startswith <- function(str1, str2) {
  if (!length(str2) == 1) {
    stop("expecting a length 1 string for 'str2'")
  }
...
stopIfNoLockfile <- function(project) {
  ...
  if (!file.exists(path)) {
    stop(
      "This project does not have a lockfile. (Have you called 'packrat::snapshot()' yet?)",
      call. = FALSE
    )
  }
...
stopIfNotPackified <- function(project) {
  if (!checkPackified(project, quiet = TRUE)) {
    if (identical(project, getwd())) {
      stop(
        "This project has not yet been packified.\nRun 'packrat::init()' to init packrat.",
        call. = FALSE
      )
    } else {
      stop(
...
setSvnIgnore <- function(svn, dir, ignores) {
  ...
  system(
    paste(svn, "propset", "svn:ignore", shQuote(ignores), "."),
    intern = TRUE
  )
}
...
updateSvnIgnore <- function(project, options) {
  ...
  add <- unique(c(add, relNewLibraryDir(), relOldLibraryDir()))
...
getInstalledPkgInfo <- function(packages, installed.packages, ...) {
  ...
  if (length(missingFromLib)) {
    warning(
      "The following packages are not installed in the current library:\n- ",
