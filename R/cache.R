                                        ...
                                        cacheDir = cacheLibDir())
{
  ...
  if (file.exists(cachedPackagePath)) {
    message("Replacing already-cached copy of package at '", cachedPackagePath, "'.")
  ...
  message("Attempting to rename '", packagePath, "' to '", cachedPackagePath, "'.")
  ...
  message("Copying '", packagePath, "' to '", tempPath, "'.")
  if (all(dir_copy(packagePath, tempPath))) {
    ...
    # populated the cache
    if (file.exists(cachedPackagePath)) {
      message("Target already exists by the time the copy completed.")
      return(symlinkPackageToCache(packagePath, cachedPackagePath))
    }

    # attempt to rename to target path
    message("Renaming copy to target location.")
