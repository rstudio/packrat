...
test_that("authDownloadAdvice offers sound advice", {
  ...
  expect_true(grepl(
    ...
    authDownloadAdvice("github", TRUE, "renv")(),
    fixed = TRUE
  ))
  ...
  expect_true(grepl(
    ...
    authDownloadAdvice("gitlab", TRUE, "httr")(),
    fixed = TRUE
  ))

  # With no available auth methods
  expect_true(grepl(
    "Packrat is not configured to use an auth-capable download method. Try setting the option packrat.authenticated.downloads.use.renv to TRUE, or installing the httr package.",
    authDownloadAdvice("bitbucket", TRUE, "internal")(),
    fixed = TRUE
  ))

  # Expected auth token (GitHub) present
  expect_true(grepl(
    "GITHUB_PAT found; check that it is correct.",
    authDownloadAdvice("github", TRUE, "renv")(),
    fixed = TRUE
  ))
  ...
  expect_true(grepl(
    "BITBUCKET_USERNAME and BITBUCKET_PASSWORD environment variables not found.",
    authDownloadAdvice("bitbucket", FALSE, "httr")(),
    fixed = TRUE
  ))
})
