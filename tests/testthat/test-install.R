test_that("The default list of environment variables is masked correctly", {
  ...
  found_in_output <- sapply(names(new_envvars), function(x) {
    any(grepl(x, subprocess_output))
  })
...
test_that("Git and user-specified variables can be masked while other variables still appear", {
  ...
  # Check masked vars
  not_expected <- sapply(masked_names, function(x) {
    any(grepl(x, subprocess_output))
  })
  ...
  # Check unmasked var
  expect_true(
    any(grepl(unmasked_name, subprocess_output)),
    info = print(subprocess_output)
  )
})
